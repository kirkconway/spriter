...
public class SpriterAnimation {
	...
	public DrawInstruction[] createDrawInstructions(SpriterKeyFrame key, float xOffset, float yOffset) {
		...
		for (int o = 0; o < key.getObjects().length; o++) {
			...
			drawInstructions[o] = new DrawInstruction(new Reference(obj.getFolder(), obj.getFile()), obj.getX()
					+ xOffset, obj.getY() + yOffset, obj.getPivotX(), obj.getPivotY(),obj.getScale_x(),obj.getScale_y(), obj.getAngle(), obj.getAlpha());
		}
	...
	public DrawInstruction[] createDrawInstructions(SpriterKeyFrame key1, SpriterKeyFrame key2,
		...
		for (int o = 0; o < key1.getObjects().length; o++) {
			...
			float scale_x = calculateInterpolation(obj1.getScale_x(), obj2.getScale_x(), key1.getStartTime(), key2StartTime,
					currentAnimationTime);
			
			float scale_y = calculateInterpolation(obj1.getScale_y(), obj2.getScale_y(), key1.getStartTime(), key2StartTime,
					currentAnimationTime);
			
			float rotation = calculateInterpolation(obj1.getAngle(), obj2Angle, key1.getStartTime(), key2StartTime,
					currentAnimationTime);
			float alpha = calculateInterpolation(obj1.getAlpha(), obj2.getAlpha(), key1.getStartTime(), key2StartTime,
					currentAnimationTime);
			...
			drawInstructions[o] = new DrawInstruction(new Reference(obj1.getFolder(), obj1.getFile()), x+xOffset, y+yOffset,
					obj1.getPivotX(), obj1.getPivotY(), scale_x, scale_y, rotation, alpha);
		}
